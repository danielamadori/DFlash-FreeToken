...
def register_openai_routes(
    ...
    async def v1_chat_completions(req: ChatCompletionRequest, request: Request):
        ...
        if (gate := _maintenance_gate(state)) is not None:
            return gate
        if (refusal := _model_refusal(req, state)) is not None:
            return refusal
...
async def handle_chat_completion(
    ...
    try:
        ...
        spec = stamp_cache_ns(
            chat_request_to_genspec(req, model_sampling, default_max_tokens=default_max_tokens),
            request,
            req.model,
        )
...
def _model_refusal(req: Any, state: Any) -> JSONResponse | None:
    """Refuse a request for a model this engine does not serve. No fallback.

    This engine used to answer ANY name -- ask it for gpt-4 and it returned 200 -- so a request
    that landed here by a routing mistake came back as a normal answer, generated by whatever
    was loaded. Naming the real model in the reply (see _answering_model) made the swap visible;
    it did not stop it, and a plausible text from the wrong model is indistinguishable from a
    right one to everything downstream that is not reading the model field.

    Daniel's call, 2026-09-15: refuse, no fallback. The refusal names what IS served, because
    the caller's next move is to ask for that, and an error that does not say what would have
    worked costs a round trip to find out.

    Matches OpenAI's shape for this case: 404, code ``model_not_found``.
    """
    chiesto = getattr(req, "model", None)
    servito = _served_model_name(state)
    if not chiesto or not servito or chiesto == servito:
        return None
    return create_error_response(
        f"The model `{chiesto}` does not exist. This node serves `{servito}`.",
        status_code=404,
