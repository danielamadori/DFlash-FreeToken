...
# The content-part types this server accepts on the way in. A part named anything else is
# refused, never dropped: a silently discarded image answers a question the caller did not
# ask, and reads as a bad model instead of an unsupported request.
#
# WHAT A NODE ADVERTISES FOLLOWS FROM WHAT IT SERVES, not from this list. Accepting the part
# here only means the wire shape is understood; whether the loaded checkpoint has an encoder
# to answer with is ``config.served_modalities``, which ``_resolve_images`` consults through
# ``image_reject_reason``. ``/props`` (server/node_metrics.py) reads the same field, so a node
# cannot announce an ability its own front door refuses.
