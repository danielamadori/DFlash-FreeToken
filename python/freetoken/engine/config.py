...
class EngineConfig:
    ...
    # Speculative Decoding via DFlash draft model
    spec_draft_model: str | None = None
    spec_block_size: int = 5
    spec_draft_dtype: str = "bfloat16"
    ...
    def model_config(self) -> ModelConfig:
        ...
        for key in set(ENCODER_SECTIONS) | {e.config_key for e in self.model_spec.encoders}:
            # Only a section that IS there needs hiding, and asking first is what lets a GGUF
            # start: its config is the frozen GgufConfigShim, built from the file header, which
            # carries no encoder section at all -- assigning None to one raises
            # FrozenInstanceError and the engine never reaches the model.
            if key not in built and getattr(hf_config, key, None) is not None:
