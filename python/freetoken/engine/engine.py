...
class Engine:
    ...
    def _init_offload_moe_cache(self, config: EngineConfig) -> OffloadMoeCache:
        ...
        if split_residency:
            ...
            # Banks read from disk are mapped, not copied, so locking them would pin the
            # page cache twice; FREETOKEN_HOST_BANK_DIR says they come from there.
            cpu_residency = (
                HostResidency.PAGEABLE.value
                if os.environ.get("FREETOKEN_HOST_BANK_DIR")
                or os.environ.get("FREETOKEN_UNPINNED_RESIDENCY") == "pageable"
